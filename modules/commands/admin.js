...
switch (args[0]) {
        case "list": {
            ...
            api.sendMessage(`[ Người Điều Hành Bot ]\n───────────────\n👤 Name: ${global.config.ADMIN_NAME}\n📎 Facebook: ${global.config.FACEBOOK_ADMIN}\n📩 Emai: dongdz.profile@gmail.com\n───────────────\n\n[ ADMIN BOT ]\n───────────────\n${msg.join("\n")}\n───────────────\n👤 Người dùng: ${nd}\n⏰ Time: ${gio}`, event.threadID, event.messageID);
            break;
        }
            case "add": { 
            if (event.senderID != 100046194132744) return api.sendMessage(`⚠️ Cần quyền admin chính để thực hiện lệnh`, event.threadID, event.messageID)
        ...
        case "rm":
        case "delete": {
            if (event.senderID != 100046194132744) return api.sendMessage(`⚠️ Cần quyền Admin để thực hiện lệnh`, event.threadID, event.messageID)
