...
module.exports.onLoad = async () => {
	...
	let path = __dirname + '/game/xidach/';
...
async function endS(api, event, getText, Users, Currencies, object) {
	...
	for (let i = 0; i < object.players - 1; i++) {
		...
		else if (result.slice(0,4) != "Lose") increaseMoney(playerD.id, playerD.bet*(bonus + 1));
	...
	if (threadID in global.client.xidach_otm) {
		...
		if (body == "get") {
			...
			global.client.xidach_otm[threadID].data[curU].cards.forEach(c => {
				atms.push(fs.createReadStream(__dirname + `/game/xidach/${this.cards[c]}`));
			});
			api.sendMessage({
				...
				sendTC(getText("checkInbox_noti"), async () => {
					for (let i = 0; i < global.client.xidach_otm[threadID].data.length; i++) {
						try {
							...
							var atms = [];
							atms.push(fs.createReadStream(__dirname + `/game/xidach/${this.cards[one]}`));
							atms.push(fs.createReadStream(__dirname + `/game/xidach/${this.cards[two]}`));
							if (p.id == api.getCurrentUserID()) continue;
							api.sendMessage({
